...
def plot_book_stats(page_stats, top_k_features=None, output_path=None):
    categories = page_stats['category_id'].unique()
    ...
    for idx, (metric, title) in enumerate(metrics):
        ...
        for page_type in page_types:
            ...
            if not type_pages.empty:
                ...
                ax.scatter(
                    ...
                    label=f"Page Type: {page_type}" if idx == 0 else None 
                )
        ...
        if idx == 0:
            ...
            category_count = min(len(categories), len(handles))
            category_handles = handles[:category_count]
            category_labels = labels[:category_count]
            
            if category_handles:
                category_legend = ax.legend(category_handles, category_labels, 
                                          loc='upper left', title='Categories')
                ax.add_artist(category_legend)
          
            page_type_handles = handles[category_count:]
            page_type_labels = labels[category_count:]
            
            if page_type_handles:
                ax.legend(page_type_handles, page_type_labels, 
