...
class BookBERTMultiVision(BookBERTfusion):
    ...
    def forward(self, fusion_features, attention_mask):
        ...
        for bb_name, bb_features in fusion_features.items():
            ...
            processed = torch.zeros((batch_size, seq_len, self.bert_input_dim), device=bb_features.device, dtype=bb_features.dtype)
            normalized_valid = self.batch_norms[bb_name](valid_features)
            projected = self.projections[bb_name](normalized_valid)
            processed[mask] = projected
                
            processed_features.append(processed)
        
        # token_indices_batch = self.token_indices.unsqueeze(0).repeat(batch_size, 1)
        ...
        combined_features = stacked_features.view(batch_size, seq_len * backbone_count, -1)

        expanded_mask = attention_mask.unsqueeze(2).expand(-1, -1, backbone_count)
        expanded_mask = expanded_mask.reshape(batch_size, seq_len * backbone_count)
        ...
        reshaped_output = sequence_output.view(batch_size, seq_len, backbone_count, -1)
