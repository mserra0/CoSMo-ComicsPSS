...
from utils.sliding_window import evaluate_with_sliding_window
...
def main(run, gpu_id = 0,train=True, precompute = False, lr = 1e-4, dropout_p=0.4, epochs = 10, batch_size=32, model_id='openai/clip-vit-large-patch14', 
    ...
    if train:
        ...
        try:
            ...
            model.to(device)
            print(f"Loaded pre-trained model from {checkpoint_dir}/best_{model_name}.pt")
        except Exception as e:
            print(f'No model to load from {checkpoint_dir}/best_{model_name}.pt', e)
    ...
    if window_size is not None:
        print(f"Using sliding window evaluation with window_size={window_size}, stride={stride}")
        results = evaluate_with_sliding_window(
            model=model,
        ...
        # Extract results
        book_mndd_scores = results['book_mndd_scores']
        docs_prec = results['docs_prec']
        docs_recall = results['docs_recall']
        docs_f1 = results['docs_f1']
        docs_sq = results['docs_sq']
        docs_pq = results['docs_pq']
        ...
        viz_output_dir = f"{out_dir}/VisionOnly/Visualizations"
        perfect_dir = os.path.join(viz_output_dir, "Perfect")
        wrong_dir = os.path.join(viz_output_dir, "Wrong")
        os.makedirs(perfect_dir, exist_ok=True)
        os.makedirs(wrong_dir, exist_ok=True)

        print("Evaluating model and generating visualizations...")
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm.tqdm(test_loader, desc="Evaluation")):
