...
def create_comic_dataset(root_dir, json_annot, dataset_path):
            ...
            for item in book.get(category_key, []):
                for page in range(item["page_start"], item["page_end"] + 1):
                    ...
                    if os.path.exists(image_path):
                        if page == item["page_start"] and category_key in ["stories"]:
                            labels.append("First-Page")
                        else:
                            labels.append(label)
    ...
    def load_image(examples, indices):
        examples["image"] = [
            Image.open(path).convert("RGB") for path in examples["image"]
        ]
        return examples
    ...
    # Create features including ClassLabel without casting
    ...
    # Create a new dataset with the correct features
    comic_dataset = Dataset.from_dict(
        comic_dataset.to_dict(), 
        features=features
    )
