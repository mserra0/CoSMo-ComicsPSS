                    ...
                    for page in range(item["page_start"], item["page_end"] + 1):
                        ...
                        if os.path.exists(image_path):
                            try:
                                ...
                                # Check if this is the first page of a story or textstory
                                if page == item["page_start"] and category_key in ["stories", "textstories"]:
                                    valid_labels.append("First-Page")
                                else:
...
class SigLIPLinearProbe(nn.Module):
    def __init__(self, siglip_model, num_classes, hidden_dim=512, dropout_p=0.3):
        super(SigLIPLinearProbe, self).__init__()
        self.siglip_model = siglip_model
        self.feature_dim = siglip_model.config.vision_config.hidden_size
    ...
    def forward(self, pixel_values):
        with torch.no_grad():
            features = self.siglip_model.vision_model(pixel_values=pixel_values).pooler_output
...
def train_model(run, model, train_loader, val_loader, device, num_epochs=10, lr=1e-4, out_dir=''):
    ...
    for epoch in epoch_pbar:
        ...
        if val_f1 > best_val_f1:  
            best_val_f1 = val_f1
            torch.save(model.state_dict(), f'{out_dir}/bestSiglipLPbase.pth')
...
def main(run, train=True, lr = 1e-4, dropout_p=0.3, epochs = 10, batch_size=32):
    ...
    train_json = "/home-local/mserrao/PSSComics/multimodal-comic-pss/EncoderClassifier/data/comics_train.json"
    val_json = "/home-local/mserrao/PSSComics/multimodal-comic-pss/EncoderClassifier/data/comics_val.json"
    test_json = "/home-local/mserrao/PSSComics/multimodal-comic-pss/EncoderClassifier/data/comics_test.json"
    cache_dir = '/home-local/mserrao/PSSComics/multimodal-comic-pss/CLIP&SigLIP/LinearProbe/data'
    out_dir = '/home-local/mserrao/PSSComics/multimodal-comic-pss/CLIP&SigLIP/LinearProbe/data/out'
    ...
    model = SigLIPLinearProbe(siglip_model, num_classes, dropout_p=dropout_p).to(device)
    model = torch.compile(model) 
    if train:
        model = train_model(run, model, train_loader, val_loader, device, num_epochs=epochs, lr=lr, out_dir=out_dir)
    else:
        model.load_state_dict(torch.load(f'{out_dir}/bestSiglipLPbase.pth', weights_only=True))
        print(f"Loaded pre-trained model from {out_dir}/bestSiglipLPbase.pth")
    ...
    image_path = f"{out_dir}/normCM_SigLIP.png"
    plt.savefig(image_path)

    wandb.log({"norm_confusion_matrix_SigLIP": wandb.Image(image_path)})
...
if __name__ == '__main__':
    ...
    run = wandb.init(
        project="ClipLP",
        name="SigLIPSimpleMLP",
        config={
            "lr": 5e-5,
            "architecture": "SigLIPLinearProbe",
            "dataset": "DCM small",
            "epochs": 20,
            "dropout": 0.3,
            "batch_size": 248,
